 ...
 * Purpose:  Trying to implement functions in place of 
 *           some code such as initializing the game and displaying the opening 
 *           Hangman 
 *           
 */
...
int main(int argc, char** argv) {
    ...
    //Declare Variables
    fstream in; //Word File
    string word, wrngAns;
    char choice, ltr1, ltr2, ltr3, ltr4, ltr5, ltr6, mode,
         ans1, ans2, ans3, ans4, ans5, ans6;
    unsigned int numLtrs, strike, linNum;
    float score;
    bool status;
...
//            in.open("ListOfWords.txt", ios::in); //Reading in random words from a word file
//            linNum = (time(0)%19)+1;   //Generates a random number and it will find a word in that 
    ...
    if (choice == 'y' || choice == 'Y') { //Tests whether the user wants to continue with the game, 
        ...
        for (int j = 0; strike < 6; j++) { //Tests that the user still has less than 6 strikes, else 
            ...
            switch(status) { //switch statement that tests whether status is true/false
                ...
                case 0: { //Tells user their guess was right
                    score += 5;
...
int gmStrt(char mode, string word) {
    fstream in; //Word File
    unsigned int numLtrs =(time(0)%4)+3;;
    word = "";
    char choice;
